...
<div class="pulv-card" style="border:1px solid var(--ds-border);border-radius:8px;margin-bottom:12px;overflow:hidden;">
    <div style="background:linear-gradient(135deg,#e8f5e9,#f1faf3);padding:10px 14px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--ds-border);">
        ...
        <div style="flex:1;">
            ...
            <span style="font-size:11px;color:#666;margin-left:10px;">
                <?php if (!empty($p->pulv_dosis)): ?>Dosis: <?= htmlspecialchars((string) $p->pulv_dosis) ?> &bull; <?php endif; ?>Jml: <?= intval($p->pulv_qty) ?> bks
            </span>
        </div>
    ...
    <div style="padding:8px 14px;">
        ...
        <div style="<?= empty($items) ? '' : 'margin-top:6px;' ?>padding:6px 10px;background:#f8f9fa;border-radius:4px;font-size:12px;color:#333;">
        ...
        <p class="text-muted" style="margin:0;font-size:12px;">
            <i class="fa fa-info-circle"></i> Isi racikan belum diisi.
        </p>
        <?php endif; ?>
    </div>
</div>
...
<p class="text-muted" style="margin:10px 0 0;font-size:13px;">
    <i class="fa fa-info-circle"></i> Belum ada racikan. Klik <strong>Tambah Racikan</strong> untuk menginput racikan baru.
</p>
<?php endif; ?>
